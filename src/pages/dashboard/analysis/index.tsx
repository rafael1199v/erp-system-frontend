...
export default function Analysis() {
	...
	const [productWithStockItems, setProductWithStockItems] = useState<ProductStock[]>();

	useEffect(() => {
		const fecthProductStock = async () => {
			const response = await companyService.getProductStock(companyId || "-1");
			const productStockList: ProductStock[] = response.data;
			setProductWithStockItems(productStockList);
		};
			...
			<div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-none shadow-none">
				<div>
					...
					<Text variant="body2" className="text-muted-foreground">
						Explora las metricas para entender las tendencias y mejorar tu inventario
					</Text>
				</div>
				...
				<div className="xl:col-span-1 h-full">
					<div className="flex flex-col xl:flex-col md:flex-row gap-4 h-full">
						<Card className="flex-1">
							<CardHeader className="flex flex-row items-center justify-between pb-2">
								<CardTitle>
									<Text variant="subTitle2">Total de productos</Text>
								</CardTitle>
							...
							<CardContent>
								<Title as="h3" className="text-xl">
									{productWithStockItems?.length ?? 0}
								</Title>
						...
						<Card className="flex-1">
							<CardHeader className="flex flex-row items-center justify-between pb-2">
								<CardTitle>
									<Text variant="subTitle2">Total de stock disponible</Text>
								</CardTitle>
							...
							<CardContent>
								<Title as="h3" className="text-xl">
									{productWithStockItems?.reduce((acc, product) => acc + product.totalStock, 0)}
								</Title>
								{/* <div className="flex flex-row gap-2 items-center">
						...
						<AlertInv
							title="Algunos productos tiene stock bajo"
							description="Revisa la seccion de productos para ver si es que puedes agregar nuevos o realizar otro tipo de procedimientos"
						/>
						{/* <Card className="flex-1">
							<CardHeader className="flex flex-row items-center justify-between pb-2">
								<CardTitle>
